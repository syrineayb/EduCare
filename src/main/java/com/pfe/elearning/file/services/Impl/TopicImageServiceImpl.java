...
@Service("TopicImageServiceImpl")
public class TopicImageServiceImpl implements FileService {
    private final Path imageLocation;

    @Autowired
    public TopicImageServiceImpl(FileStorageProperties fileStorageProperties) {
        this.imageLocation= Paths.get(fileStorageProperties.getUploadImgTopicsDir()).toAbsolutePath().normalize();
