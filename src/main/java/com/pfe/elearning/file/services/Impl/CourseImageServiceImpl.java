...
public class CourseImageServiceImpl implements  FileService {
    private final Path imageLocation;

    @Autowired
    public CourseImageServiceImpl(FileStorageProperties fileStorageProperties) {
        this.imageLocation= Paths.get(fileStorageProperties.getUploadImgCoursesDir()).toAbsolutePath().normalize();
